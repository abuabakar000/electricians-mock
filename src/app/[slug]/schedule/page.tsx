...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return {
    title: `Schedule Service | ${leadData.title}`,
    description: `Schedule your professional service with ${leadData.title} online. Choose your preferred date and time slot for our expert technicians.`,
    icons: {
...
export default async function LeadSchedulePage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const leadData = getLeadData(slug);
        ...
        <section className="relative min-h-[50vh] flex items-center justify-center overflow-hidden border-b border-zinc-800/60">
          ...
          <div className="relative z-10 text-center px-6 max-w-5xl mx-auto pt-24 pb-12">
            ...
            <h1 className="text-5xl md:text-6xl lg:text-7xl font-light text-white tracking-tighter leading-[1.05] mb-8">
              ...
              <span 
                style={{
                  background: leadData.slug === "default"
                    ? "linear-gradient(to right, #fef3c7, #f59e0b)"
                    : `linear-gradient(to right, #ffffff, ${leadData.primaryColor})`,
